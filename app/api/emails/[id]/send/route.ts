...
export async function POST(
  ...
  try {
    ...
    const apiKeyString = await env.SITE_CONFIG.get("RESEND_API_KEY")
    ...
    const apiKeyLines = apiKeyString
      .split(/[\n,]+/)
      .map(k => k.trim())
      .filter(Boolean)
    ...
    if (apiKeyLines.length === 0) {
      return NextResponse.json(
        { error: "Resend 发件服务未配置有效的 API Key，请联系管理员" },
        { status: 500 }
      )
    }

    const senderDomain = email.address.split('@')[1]
    let apiKey = ""

    // Try to find a domain-specific key
    for (const line of apiKeyLines) {
      if (line.includes(':')) {
        const [domain, key] = line.split(':').map(s => s.trim())
